...
def generate_similar(
    source: Annotated[
        Path,
        typer.Argument(
            help="Source image previously generated by image-gen (reads its EXIF metadata)",
            exists=True,
            file_okay=True,
            dir_okay=False,
        ),
    ...
    """Re-generate new images with a source image's parameters.

    Reads the generation metadata embedded in SOURCE's EXIF (written by `generate`
    or `generate-var`) and reuses every parameter as-is. By default the seed is
    re-randomized per image; pass --keep-seed to reuse the source's exact seed
    instead. Any parameter can be overridden with the same flags as `generate`.
    ...
    # Validate overridden scheduler / IP-Adapter preset up front.
    ...
    # Resolve every other parameter: override if given, else the source's value.
    ...
    # Initialize pipeline once; reused for every one of the `count` generations.
    ...
    for i in range(count):
        ...
        if vary is not None:
            ...
            iter_negative = variation.negative_prompt if variation.negative_prompt is not None else resolved_negative
